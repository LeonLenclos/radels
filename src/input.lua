...
function input.update(dt)
   ...
   if #joysticks >= 2  then
	  -- push buttons
	  input.p1_shoot =  joysticks[1]:isGamepadDown('a')
	  input.p1_action = joysticks[1]:isGamepadDown('b')

	  input.p2_shoot =  joysticks[2]:isGamepadDown('a')
	  input.p2_action = joysticks[2]:isGamepadDown('b')

	  -- joysticks
	  input.p1_up =  joysticks[1]:getGamepadAxis("lefty") < 0
	  input.p1_down = joysticks[1]:getGamepadAxis("lefty") > 0
	  input.p1_left = joysticks[1]:getGamepadAxis("leftx") < 0
	  input.p1_right = joysticks[1]:getGamepadAxis("leftx") > 0

	  input.p2_up = joysticks[2]:getGamepadAxis("lefty") < 0
	  input.p2_down = joysticks[2]:getGamepadAxis("lefty") > 0
	  input.p2_left = joysticks[2]:getGamepadAxis("leftx") < 0
	  input.p2_right = joysticks[2]:getGamepadAxis("leftx") > 0

	  -- coin
	  input.insert_coin = joysticks[1]:isGamepadDown('x')
